...
{
   index = index; //this is here to bypass compiler warnings about unused arguments
   return (!isGrassType(COMBOTYPE(Link.getX(), Link.getY() + 15))
           || !isGrassType(COMBOTYPE(Link.getX() + 15, Link.getY() + 15)) || Link.getZ() > 8);
}
...
void dNayrusLoveShield::realdraw(BITMAP *dest, int draw_what)
{
   ...
   if (((Link.getNayrusLoveShieldClk() & 0x20) || (Link.getNayrusLoveShieldClk() & 0xF00)) && (!flickering
