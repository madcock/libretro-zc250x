                                   ...
                                   uint8_t skiprows = 0)
   {
      ...
      switch (flip)
      {
         ...
         case 3:
            for (int j = h - 1; j >= 0; j--)
               for (int k = w - 1; k >= 0; k--)
                  overtiletranslucent16(_Dest, tile + (j * TILES_PER_ROW) + k, x + ((w - 1) - k) * 16, y + ((h - 1) - j) * 16, color,
                                        flip, opacity);
                                  ...
                                  uint8_t skiprows = 0)
   {
      ...
      switch (flip)
      {
         ...
         case 3:
            for (int j = h - 1; j >= 0; j--)
               for (int k = w - 1; k >= 0; k--)
                  puttiletranslucent16(_Dest, tile + (j * TILES_PER_ROW) + k, x + ((w - 1) - k) * 16, y + ((h - 1) - j) * 16, color, flip,
                                       opacity);
...
void do_arcr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
   ...
   if (sdci[12]) //closed
   {
      if (sdci[13]) //filled
      {
         ...
         line(prim_bmp, cx + xoffset, cy + yoffset, cx + xoffset + fixtoi(fixcos(-sa)*r), cy + yoffset + fixtoi(fixsin(-sa)*r),
              color);
         line(prim_bmp, cx + xoffset, cy + yoffset, cx + xoffset + fixtoi(fixcos(-ea)*r), cy + yoffset + fixtoi(fixsin(-ea)*r),
              color);
      ...
      else
      {
         ...
         line(bmp, cx + xoffset, cy + yoffset, cx + xoffset + fixtoi(fixcos(-sa)*r), cy + yoffset + fixtoi(fixsin(-sa)*r),
              color);
         line(bmp, cx + xoffset, cy + yoffset, cx + xoffset + fixtoi(fixcos(-ea)*r), cy + yoffset + fixtoi(fixsin(-ea)*r),
              color);
      }
   }
   else
...
void do_fasttiler(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
   ...
   if (opacity < 128)
      overtiletranslucent16(bmp, sdci[4] / 10000, xoffset + (sdci[2] / 10000), yoffset + (sdci[3] / 10000), sdci[5] / 10000,
                            0, opacity);
   else
...
void do_drawintr(BITMAP *bmp, int *sdci, int xoffset, int yoffset)
{
   ...
   if (w > 0 && h > 0) //stretch
   {
      BITMAP *pbmp = create_sub_bitmap(prim_bmp, 0, 0, text_length(get_zc_font(font_index), numbuf) + 1,
                                       text_height(get_zc_font(font_index)));
      clear_bitmap(pbmp);
