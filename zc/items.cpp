   ...
   if (anim)
   {
      ...
      if (extend > 2)
      {
         if (o_tile / TILES_PER_ROW == (o_tile + txsz * aframe) / TILES_PER_ROW)
            ...
            tile = o_tile + (txsz * aframe) + ((tysz - 1) * TILES_PER_ROW) * ((o_tile + txsz * aframe) / TILES_PER_ROW) -
...
int select_dropitem(int item_set, int x, int y)
{
   ...
   for (int k = 0; k < 11; ++k)
   {
      ...
      if (k > 0)
      {
         ...
         if ((!get_bit(quest_rules, qr_ENABLEMAGIC) || (game->get_maxmagic() <= 0))
               && (itemsbuf[current_item].family == itype_magic))
            current_chance = 0;
   ...
   for (int k = 10; k >= 0; --k)
   {
      ...
      if ((!get_bit(quest_rules, qr_ENABLEMAGIC) || (game->get_maxmagic() <= 0)) && (current_item >= 0
            && itemsbuf[current_item].family == itype_magic))
         current_chance = 0;
   ...
   if (drop_item >= 0 && itemsbuf[drop_item].family == itype_fairy)
   {
      ...
      {
         if ((itemsbuf[items.spr(j)->id].family == itype_fairy) && ((abs(items.spr(j)->x - x) < 32)
               || (abs(items.spr(j)->y - y) < 32)))
         {
            drop_item = -1;
            break;
         }
