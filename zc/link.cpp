...
{
   int power = directWpn > -1 ? itemsbuf[directWpn].power : (current_item_power(attack == wWand ? itype_wand : attack ==
               wHammer ? itype_hammer : itype_sword));

   // Multiply it by the power of the spin attack/quake hammer, if applicable.
   power *= (spins > 0 ? itemsbuf[current_item_id(attack == wHammer ? itype_quakescroll : (spins > 5
                                                         || current_item_id(itype_spinscroll) < 0) ? itype_spinscroll2 : itype_spinscroll)].power : 1);
   return power;
}
...
attack:
   ...
   if (attackclk || action == attacking)
   {
      ...
      */
      int itype = (attack == wFire ? itype_candle : attack == wCByrna ? itype_cbyrna : attack == wWand ? itype_wand : attack
                   == wHammer ? itype_hammer : itype_sword);
      ...
      {
         if ((attack == wSword || attack == wWand || ((attack == wFire || attack == wCByrna) && itemsbuf[itemid].wpn))
               && wpnsbuf[itemsbuf[itemid].wpn].tile)
         {
            ...
            || (attack == wHammer && attackclk <= 30))
      {
         if (!invisible)
         {
            ...
            if ((game->get_canslash() && (attack == wSword || attack == wWand || attack == wFire || attack == wCByrna))
                  && itemsbuf[itemid].flags & ITEM_FLAG4 && (attackclk < 7))
   ...
   if (action != casting)
   {
      ...
      bool inwater = iswater(MAPCOMBO(x + 4, y + 9))  && iswater(MAPCOMBO(x + 4, y + 15))
                     &&  iswater(MAPCOMBO(x + 11, y + 9)) && iswater(MAPCOMBO(x + 11, y + 15));
   ...
   if (action == landhold1 || action == landhold2)
   {
      if (holditem > -1)
      {
         if (get_bit(quest_rules, qr_HOLDITEMANIMATION))
            putitem2(dest, x - ((action == landhold1) ? 4 : 0), y + yofs - 16 - (get_bit(quest_rules, qr_NOITEMOFFSET)), holditem,
   ...
   else if (action == waterhold1 || action == waterhold2)
   {
      ...
      {
         if (get_bit(quest_rules, qr_HOLDITEMANIMATION))
            putitem2(dest, x - ((action == waterhold1) ? 4 : 0), y + yofs - 12 - (get_bit(quest_rules, qr_NOITEMOFFSET)), holditem,
                     lens_hint_item[holditem][0], lens_hint_item[holditem][1], 0);
         else
...
{
   if (action != attacking || (attack != wSword && attack != wWand && attack != wHammer && attack != wCByrna
                               && attack != wFire)
         || (attackclk <= 4))
      return;
   ...
   for (int i = 0; i < guys.Count(); i++)
   {
      ...
      if ((guys.spr(i)->hit(wx, wy, wz, wxsz, wysz, wz > 0 ? 16 : 8) && (attack != wWand
            || !get_bit(quest_rules, qr_NOWANDMELEE)))
   ...
   else if ((attack == wHammer) && ((attackclk == 15) || (spins == 1
                                    && attackclk >= 15))) //quake hammer should be spins == 1
   {
      // poundable blocks
      for (int q = 0; q < 176; q++)
         set_bit(screengrid, q, 0);
...
void LinkClass::check_slash_block(int bx, int by)
{
   ...
   if (z > 8 || attackclk ==
         SWORDCHARGEFRAME // is not charging>0, as tapping a wall reduces attackclk but retains charging
   ...
   int sworditem = (directWpn > -1
                    && itemsbuf[directWpn].family == itype_sword) ? itemsbuf[directWpn].fam_type : current_item(itype_sword);
...
void LinkClass::checkhit()
{
   ...
   if (hclk >= 40 && action == gothit)
   {
      if (((ladderx + laddery) && ((hitdir & 2) == ladderdir)) || (!(ladderx + laddery)))
      {
         for (int i = 0; i < 4; i++)
         {
            ...
            {
               case up:
                  if (hit_walkflag(x, y + (bigHitbox ? -1 : 7), 2)
                        || (int(x) & 7 ? hit_walkflag(x + 16, y + (bigHitbox ? -1 : 7), 1) : 0))    action = none;
                  else --y;
               ...
               case left:
                  if (hit_walkflag(x - 1, y + (bigHitbox ? 0 : 8), 1) || hit_walkflag(x - 1, y + 8, 1)
                        || (int(y) & 7 ? hit_walkflag(x - 1, y + 16, 1) : 0))  action = none;
                  else --x;

                  break;

               case right:
                  if (hit_walkflag(x + 16, y + (bigHitbox ? 0 : 8), 1) || hit_walkflag(x + 16, y + 8, 1)
                        || (int(y) & 7 ? hit_walkflag(x + 16, y + 16, 1) : 0)) action = none;
                  else ++x;
   ...
   if (hclk > 0 || inlikelike == 1 || action == inwind || action == drowning || inwallm || diveclk > 30
   ...
   for (int i = 0; i < Lwpns.Count(); i++)
   {
      ...
      if (!get_bit(quest_rules, qr_FIREPROOFLINK) && (scriptcoldet & 1) && (!superman
            || !get_bit(quest_rules, qr_FIREPROOFLINK2)))
      {
         int itemid = ((weapon *)(Lwpns.spr(i)))->parentitem;

         if (s->id == wFire
               && (superman ? (diagonalMovement ? s->hit(x + 4, y + 4, z, 7, 7, 1) : s->hit(x + 7, y + 7, z, 2, 2, 1)) : s->hit(this))
               &&
...
killweapon:
                        ...
                        if (ew->id != oldid || (ew->id >= wScript1
                                                && ew->id <= wScript10)) // changed type from ewX to wX... Except for script weapons
                        {
                           Lwpns.add(ew);
                           Ewpns.remove(ew);
                        }
      ...
      if (get_bit(quest_rules, qr_OUCHBOMBS))
      {
         ...
         if (((s->id == wBomb) || (s->id == wSBomb)) && s->hit(this) && !superman && (scriptcoldet & 1))
         {
            if (NayrusLoveShieldClk <= 0)
            {
               int ringpow = ringpower(((((weapon *)s)->parentitem > -1 ? itemsbuf[((weapon *)s)->parentitem].misc3 : ((
                                            weapon *)s)->power) * HP_PER_HEART));
...
bool LinkClass::checkdamagecombos(int dx1, int dx2, int dy1, int dy2, int layer, bool solid) //layer = -1, solid = false
{
   ...
   hp_mod[0] = combo_class_buf[combobuf[layer > -1 ? MAPCOMBO2(layer, dx1, dy1) : MAPCOMBO(dx1,
                                        dy1)].type].modify_hp_amount;
   hp_mod[1] = combo_class_buf[combobuf[layer > -1 ? MAPCOMBO2(layer, dx1, dy2) : MAPCOMBO(dx1,
                                        dy2)].type].modify_hp_amount;
   hp_mod[2] = combo_class_buf[combobuf[layer > -1 ? MAPCOMBO2(layer, dx2, dy1) : MAPCOMBO(dx2,
                                        dy1)].type].modify_hp_amount;
   hp_mod[3] = combo_class_buf[combobuf[layer > -1 ? MAPCOMBO2(layer, dx2, dy2) : MAPCOMBO(dx2,
                                        dy2)].type].modify_hp_amount;
   ...
   if (hp_modmin < 0)
   {
      if ((itemid < 0) || (tmpscr->flags5 & fDAMAGEWITHBOOTS) || (4 << current_item_power(itype_boots) < (abs(hp_modmin)))
            || (solid && bootsnosolid) || !checkmagiccost(itemid))
      {
         if (NayrusLoveShieldClk <= 0)
         {
...
bool LinkClass::animate(int)
{
   ...
   if (tmpscr->flags7 & fSIDEVIEW) // Sideview gravity
   {
      ...
      else if (fall <= (int)zinit.terminalv)
      {
         ...
         else if (fall + int(zinit.gravity) > 0 && fall <= 0 && can_use_item(itype_hoverboots, i_hoverboots) && !ladderx
                  && !laddery)
         {
            fall = jumping = 0;
   ...
   if (action != landhold1 && action != landhold2 && action != waterhold1 && action != waterhold2 && fairyclk == 0
         && holdclk > 0)
      holdclk = 0;
   ...
   if (diagonalMovement)
   {
      if (ladderx + laddery)
      {
         if (ladderdir == up)
         {
            if ((laddery - int(y) >= (16 + (ladderstart == dir ? ladderstart == down ? 1 : 0 : 0)))
                  || (laddery - int(y) <= (-16 - (ladderstart == dir ? ladderstart == up ? 1 : 0 : 0))) || (abs(ladderx - int(x)) > 8))
               reset_ladder();
         }
         else
         {
            if ((abs(laddery - int(y)) > 8) || (ladderx - int(x) >= (16 + (ladderstart == dir ? ladderstart == right ? 1 : 0 : 0)))
                  || (ladderx - int(x) <= (-16 - (ladderstart == dir ? ladderstart == left ? 1 : 0 : 0))))
               reset_ladder();
         }
      }
   }
   ...
   {
      if (((dtype == dBOMBED) ? DrunkUp() : dir == up) && (diagonalMovement ? x > 112 && x < 128 : x == 120) && y <= 32
            && tmpscr->door[0] == dtype)
      {
         walk = true;
         dir = up;
      ...
      if (((dtype == dBOMBED) ? DrunkDown() : dir == down) && (diagonalMovement ? x > 112 && x < 128 : x == 120) && y >= 128
            && tmpscr->door[1] == dtype)
      {
         walk = true;
         dir = down;
      ...
      if (((dtype == dBOMBED) ? DrunkLeft() : dir == left) && x <= 32 && (diagonalMovement ? y > 72 && y < 88 : y == 80)
            && tmpscr->door[2] == dtype)
      {
         walk = true;
         dir = left;
      ...
      if (((dtype == dBOMBED) ? DrunkRight() : dir == right) && x >= 208 && (diagonalMovement ? y > 72 && y < 88 : y == 80)
            && tmpscr->door[3] == dtype)
      {
         walk = true;
         dir = right;
...
{
   if (getItemFamily(itemsbuf, Bwpn & 0x0FFF) == (super ? itype_sbomb : itype_bomb) && (directWpn < 0
         || Bwpn == directWpn))
   {
...
bool LinkClass::startwpn(int itemid)
{
   ...
   switch (itemsbuf[itemid].family)
   {
      ...
      case itype_rocs:
      {
         if (!inlikelike && z == 0 && charging == 0 && !(tmpscr->flags7 & fSIDEVIEW && !ON_SIDEPLATFORM && !ladderx && !laddery)
               && hoverclk == 0)
         {
            if (!checkmagiccost(itemid))
               return false;
      ...
      case itype_whistle:
      {
         ...
         else
         {
            ...
            if (((DMaps[currdmap].flags & dmfWHIRLWIND && TriforceCount()) || DMaps[currdmap].flags & dmfWHIRLWINDRET) &&
               ...
               Lwpns.add(new weapon((fix)(where == left ? 240 : where == right ? 0 : x),
                                    (fix)(where == down ? 0 : where == up ? 160 : y),
      ...
      case itype_arrow:
      {
         ...
         Lwpns.add(new weapon((fix)wx, (fix)wy, (fix)wz, wArrow, itemsbuf[itemid].fam_type,
                              DAMAGE_MULTIPLIER * itemsbuf[itemid].power, dir, itemid, getUID()));
      ...
      case itype_brang:
      {
         ...
         Lwpns.add(new weapon((fix)wx, (fix)wy, (fix)wz, wBrang, itemsbuf[itemid].fam_type,
                              (itemsbuf[itemid].power * DAMAGE_MULTIPLIER), dir, itemid, getUID()));
      }
      break;
      ...
      case itype_cbyrna:
      {
         ...
         for (int i = 0; i < itemsbuf[itemid].misc3; i++)
            Lwpns.add(new weapon((fix)wx, (fix)wy, (fix)wz, wCByrna, i, itemsbuf[itemid].power * DAMAGE_MULTIPLIER, dir, itemid,
                                 getUID()));
...
bool LinkClass::doattack()
{
   ...
   if (attackclk >= (spins > 0 ? 8 : 14) && attack != wHammer &&
         (((attack != wSword || !current_item(itype_spinscroll) || inlikelike) && attack != wWand && attack != wFire
           && attack != wCByrna) || !((attack == wSword && isWpnPressed(itype_sword) && spins == 0) || charging > 0)))
   {
      tapping = false;
      return false;
   }
   ...
   int candleid = (directWpn > -1
                   && itemsbuf[directWpn].family == itype_candle) ? directWpn : current_item_id(itype_candle);
   int byrnaid = (directWpn > -1
                  && itemsbuf[directWpn].family == itype_cbyrna) ? directWpn : current_item_id(itype_cbyrna);
   ...
   else
   {
      ...
      if (!isWpnPressed(attack == wFire ? itype_candle : attack == wCByrna ? itype_cbyrna : attack == wWand ? itype_wand :
                        attack == wHammer ? itype_hammer : itype_sword))
         charging = 0;
   ...
   if (attackclk == 13 || (attackclk == 7 && spins > 1 && crossid >= 0 && checkmagiccost(crossid)))
   {
      ...
      bool normalbeam = (game->get_life() + (get_bit(quest_rules,
                                             qr_QUARTERHEART) ? ((HP_PER_HEART / 4) - 1) : ((HP_PER_HEART / 2) - 1)) >= templife);
...
bool LinkClass::can_attack()
{
   if (action == hopping || action == swimming || action == freeze ||
         (action == attacking && (attack != wSword || attack != wWand || !get_bit(quest_rules, qr_QUICKSWORD)) && charging != 0)
         || spins > 0)
      return false;
      ...
      if (diagonalMovement)
      {
         if (hopclk == 1) //hopping out
         {
            ...
            if (dir == up)
            {
               ...
               if (iswater(MAPCOMBO(x, y + (bigHitbox ? 0 : 8) - 1)) && !iswater(MAPCOMBO(x + 8, y + (bigHitbox ? 0 : 8) - 1))
                     && !iswater(MAPCOMBO(x + 15, y + (bigHitbox ? 0 : 8) - 1)))
                  sidestep = 1;
               else if (!iswater(MAPCOMBO(x, y + (bigHitbox ? 0 : 8) - 1)) && !iswater(MAPCOMBO(x + 7, y + (bigHitbox ? 0 : 8) - 1))
                        && iswater(MAPCOMBO(x + 15, y + (bigHitbox ? 0 : 8) - 1)))
                  sidestep = 2;
            ...
            if (dir == left)
            {
               ...
               if (iswater(MAPCOMBO(x - 1, y + (bigHitbox ? 0 : 8))) && !iswater(MAPCOMBO(x - 1, y + (bigHitbox ? 8 : 12)))
                     && !iswater(MAPCOMBO(x - 1, y + 15)))
                  sidestep = 1;
               else if (!iswater(MAPCOMBO(x - 1, y + (bigHitbox ? 0 : 8))) && !iswater(MAPCOMBO(x - 1, y + (bigHitbox ? 7 : 11)))
                        && iswater(MAPCOMBO(x - 1, y + 15)))
                  sidestep = 2;
            ...
            if (dir == right)
            {
               ...
               if (iswater(MAPCOMBO(x + 16, y + (bigHitbox ? 0 : 8))) && !iswater(MAPCOMBO(x + 16, y + (bigHitbox ? 8 : 12)))
                     && !iswater(MAPCOMBO(x + 16, y + 15)))
                  sidestep = 1;
               else if (!iswater(MAPCOMBO(x + 16, y + (bigHitbox ? 0 : 8))) && !iswater(MAPCOMBO(x + 16, y + (bigHitbox ? 7 : 11)))
                        && iswater(MAPCOMBO(x + 16, y + 15)))
                  sidestep = 2;
...
void LinkClass::do_rafting()
{
   ...
   if (!(int(x) & 15) && !(int(y) & 15))
   {
      ...
      if ((MAPFLAG(x, y) == mfRAFT_BRANCH || MAPCOMBOFLAG(x, y) == mfRAFT_BRANCH))
      {
         ...
         if (dir != left && DrunkRight() && (isRaftFlag(nextflag(x, y, right, false))
                                             || isRaftFlag(nextflag(x, y, right, true))))
         {
            dir = right;
            goto skip;
         }
...
void LinkClass::movelink()
{
   ...
   if ((!attackclk && action != attacking) || ((attack == wSword || attack == wWand)
         && get_bit(quest_rules, qr_QUICKSWORD)))
   {
      if (DrunkrBbtn())
      {
   ...
   if (can_attack() && (directWpn > -1 ? itemsbuf[directWpn].family == itype_sword : current_item(itype_sword))
         && swordclk == 0 && btnwpn == itype_sword && charging == 0)
   {
      action = attacking;
      attack = wSword;
   ...
   if (can_attack() && itemclk == 0 && btnwpn > itype_sword && charging == 0
         && btnwpn != itype_rupee) // This depends on item 0 being a rupee...
   {
      bool paidmagic = false;

      if (btnwpn == itype_wand
            && (directWpn > -1 ? (!item_disabled(directWpn) ? itemsbuf[directWpn].family == itype_wand : false) : current_item(
                   itype_wand)))
      {
         action = attacking;
         attack = wWand;
         ...
         attackclk = 0;
               ...
               && (directWpn > -1 ? (!item_disabled(directWpn) ? itemsbuf[directWpn].family == itype_hammer : false) : current_item(
                      itype_hammer)) && checkmagiccost(dowpn))
      {
         paymagiccost(dowpn);
         paidmagic = true;
         action = attacking;
         ...
         attackclk = 0;
               ...
               && (directWpn > -1 ? (!item_disabled(directWpn) ? itemsbuf[directWpn].family == itype_candle : false) : current_item(
                      itype_candle)))
      {
         action = attacking;
         attack = wFire;
         ...
         attackclk = 0;
               ...
               && (directWpn > -1 ? (!item_disabled(directWpn) ? itemsbuf[directWpn].family == itype_cbyrna : false) : current_item(
                      itype_cbyrna)))
      {
         action = attacking;
         attack = wCByrna;
...
LEFTRIGHT:
   ...
   if (DrunkRight())
   {
      ...
      {
         info = walkflag((int)x + 15 + (lsteps[int(x) & 7]), y + (bigHitbox ? 0 : 8), 1, right)
...
void LinkClass::move(int d2)
{
   ...
   bool slowcombo = (combo_class_buf[combobuf[MAPCOMBO(x + 7, y + 8)].type].slow_movement && (z == 0
                     || tmpscr->flags2 & fAIRCOMBOS)) ||
...
LinkClass::WalkflagInfo LinkClass::walkflag(int wx, int wy, int cnt, uint8_t d2)
{
   ...
   if (isdungeon() && currscr < 128 && wy < (bigHitbox ? 32 : 40) && ((diagonalMovement ? (x <= 112
         || x >= 128) : x != 120) || _walkflag(120, 24, 2))
...
void LinkClass::checkpushblock()
{
   ...
   for (int i = (get_bit(quest_rules, qr_DMGCOMBOLAYERFIX) ? 2 : 0); i >= 0; i--)
   {
      ...
      if (combo_class_buf[t].modify_hp_amount && _walkflag(bx, by, 1) && pushing > 0 && hclk < 1 && action != casting
            && !get_bit(quest_rules, qr_NOSOLIDDAMAGECOMBOS))
      {
         // Bite Link
   ...
   if (get_bit(quest_rules, qr_SOLIDBLK))
   {
      ...
      {
         case up:
            if (_walkflag(bx, by - 8, 2) && !(MAPFLAG(bx, by - 8) == mfBLOCKHOLE
                                              || MAPCOMBOFLAG(bx, by - 8) == mfBLOCKHOLE))    doit = false;

            break;

         case down:
            if (_walkflag(bx, by + 24, 2) && !(MAPFLAG(bx, by + 24) == mfBLOCKHOLE
                                               || MAPCOMBOFLAG(bx, by + 24) == mfBLOCKHOLE))   doit = false;

            break;

         case left:
            if (_walkflag(bx - 16, by + 8, 2) && !(MAPFLAG(bx - 16, by + 8) == mfBLOCKHOLE
                                                   || MAPCOMBOFLAG(bx - 16, by + 8) == mfBLOCKHOLE)) doit = false;
...
void LinkClass::checklocked()
{
   ...
   */
   ...
   for (int q = 0; q < 4; q++)
   {
      if (tmpscr->door[q] == dLOCKED || tmpscr->door[q] == dBOSS)
         found = true;
   }
...
void LinkClass::checkspecial2(int *ls)
{
   ...
   // 2.10 compatibility...
   else if (int(y) % 16 == 8 && types[0] == types[2] && (types[0] == mfFAIRY || types[0] == mfMAGICFAIRY
            || types[0] == mfALLFAIRY))
      flag = types[0];
   ...
   // * Not swallowed,
   // * Not a dried lake.
   if (water && get_bit(quest_rules, qr_DROWN) && z == 0  && fall >= 0 && !ladderx && !hoverclk && action != rafting
         && !isSwimming() && !inlikelike && !DRIEDLAKE)
   {
         ...
         !(type == cDIVEWARP && diveclk > 30))
   {
      switch (flag)
      {
         ...
         case mfRAFT_BRANCH:
            ...
            if (current_item(itype_raft) && action != rafting && action != swimhit && action != gothit && z == 0
                  && combo_class_buf[type].dock)
            {
...
bool LinkClass::dowarp(int type, int index)
{
   ...
   switch (wtype)
   {
      ...
      case wtEXIT: // entrance/exit
      {
         ...
         else
         {
            ...
            if ((type1 == cCAVE) || (type1 >= cCAVEB && type1 <= cCAVED) || (type2 == cCAVE) || (type2 >= cCAVEB
                  && type2 <= cCAVED))
            {
               reset_pal_cycling();
               putscr(scrollbuf, 0, 0, tmpscr);
            ...
            else if ((type3 == cCAVE2) || (type3 >= cCAVE2B && type3 <= cCAVE2D) || (type2 == cCAVE2) || (type2 >= cCAVE2B
                     && type2 <= cCAVE2D))
            {
               reset_pal_cycling();
               putscr(scrollbuf, 0, 0, tmpscr);
      ...
      case wtSCROLL:                                          // scrolling warp
      {
         ...
         if (!intradmap)
         {
            ...
            if (((wrx > 0 || wry > 0) || (get_bit(quest_rules, qr_WARPSIGNOREARRIVALPOINT)))
                  && (!get_bit(quest_rules, qr_NOSCROLLCONTINUE)) && (!(tmpscr->flags6 & fNOCONTINUEHERE)))
            {
               if (dlevel)
                  lastentrance = currscr;
               else
      ...
      case wtWHISTLE:                                         // whistle warp
      {
         ...
         Lwpns.add(new weapon((fix)(index == left ? 240 : index == right ? 0 : wrx),
                              (fix)(index == down ? 0 : index == up ? 160 : wry),
      ...
      case wtIWARPWAVE:                                       // insta-warps
      {
         ...
         bool cavewarp = ((type1 == cCAVE) || (type1 >= cCAVEB && type1 <= cCAVED) || (type2 == cCAVE) || (type2 >= cCAVEB
                          && type2 <= cCAVED)
                          || (type3 == cCAVE2) || (type3 >= cCAVE2B && type3 <= cCAVE2D) || (type2 == cCAVE2) || (type2 >= cCAVE2B
                                && type2 <= cCAVE2D));
   ...
   if ((DMaps[currdmap].type & dmfCONTINUE) || (currdmap == 0))
   {
      if (dlevel)
      {
         ...
         if ((wtype == wtEXIT)
               || (((wtype == wtSCROLL) && !intradmap) && ((wrx > 0 || wry > 0)
...
void LinkClass::checkscroll()
{
   ...
   if (y < 0)
   {
      ...
      if (get_bit(quest_rules, qr_SMARTSCREENSCROLL) && (!(tmpscr->flags & fMAZE)) && action != inwind && action != scrolling
            && !(tmpscr->flags2 & wfUP))
      {
...
int LinkClass::lookahead(int destscr,
                         int d2)                       // Helper for scrollscr that gets next combo on next screen.
{
...
void getitem(int id, bool nosound)
{
      ...
      if (itemsbuf[id].count != -1)
      {
         ...
         if (itemsbuf[id].amount & 0x3FFF)
         {
            if (itemsbuf[id].amount & 0x8000)
               game->set_dcounter(
                  game->get_dcounter(itemsbuf[id].count) + ((itemsbuf[id].amount & 0x4000) ? -(itemsbuf[id].amount & 0x3FFF) :
                        itemsbuf[id].amount & 0x3FFF), itemsbuf[id].count);
            else
            {
               if (itemsbuf[id].amount >= 16385 && game->get_counter(0) <= itemsbuf[id].amount - 16384)
                  ...
                  game->set_counter(zc_min(game->get_counter(itemsbuf[id].count) + ((itemsbuf[id].amount & 0x4000) ? -
                                           (itemsbuf[id].amount & 0x3FFF) : itemsbuf[id].amount & 0x3FFF), game->get_maxcounter(itemsbuf[id].count)),
                                    itemsbuf[id].count);
            }
         }
      }
   ...
   switch (itemsbuf[id & 0xFF].family)
   {
      ...
      case itype_fairy:
         ...
         game->set_life(zc_min(game->get_life() + (itemsbuf[id].flags & ITEM_FLAG1 ? (int)(game->get_maxlife() *
                               (itemsbuf[id].misc1 / 100.0)) : ((itemsbuf[id].misc1 * HP_PER_HEART))), game->get_maxlife()));
         game->set_magic(zc_min(game->get_magic() + (itemsbuf[id].flags & ITEM_FLAG2 ? (int)(game->get_maxmagic() *
                                (itemsbuf[id].misc2 / 100.0)) : ((itemsbuf[id].misc2 * MAGICPERBLOCK))), game->get_maxmagic()));
         break;
...
void takeitem(int id)
{
   ...
   if (itemsbuf[id].count != -1)
   {
      ...
      {
         if (itemsbuf[id].amount & 0x8000)
            game->set_dcounter(game->get_dcounter(itemsbuf[id].count) - ((itemsbuf[id].amount & 0x4000) ? -
                               (itemsbuf[id].amount & 0x3FFF) : itemsbuf[id].amount & 0x3FFF), itemsbuf[id].count);
         else game->set_counter(game->get_counter(itemsbuf[id].count) - ((itemsbuf[id].amount & 0x4000) ? -
                                   (itemsbuf[id].amount & 0x3FFF) : itemsbuf[id].amount & 0x3FFF), itemsbuf[id].count);
      }
   }
...
void LinkClass::checkitems(int index)
{
   ...
   if (pickup & ipHOLDUP)
   {
      ...
      if (get_bit(quest_rules, qr_OLDPICKUP) || ((tmpscr[tmp].room == rSP_ITEM || tmpscr[tmp].room == rRP_HC
            || tmpscr[tmp].room == rTAKEONE) && (pickup & ipONETIME2)))
         fadeclk = 66;
...
bool LinkClass::refill()
{
   ...
   {
      refill_heart_stop = zc_min(potion_life + (itemsbuf[refill_why].flags & ITEM_FLAG1 ? int(game->get_maxlife() *
                                 (itemsbuf[refill_why].misc1 / 100.0)) : ((itemsbuf[refill_why].misc1 * HP_PER_HEART))), game->get_maxlife());
      refill_magic_stop = zc_min(potion_magic + (itemsbuf[refill_why].flags & ITEM_FLAG2 ? int(game->get_maxmagic() *
                                 (itemsbuf[refill_why].misc2 / 100.0)) : ((itemsbuf[refill_why].misc2 * MAGICPERBLOCK))), game->get_maxmagic());
   }
