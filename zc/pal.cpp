...
{
   bool newstate = !existslight
                   && (setnaturaldark ? ((TheMaps[currmap * MAPSCRS + currscr].flags & fDARK) != 0) : naturaldark);

   if (darkroom != newstate)
   {
      fade((Link.getSpecialCave() > 0) ? (Link.getSpecialCave() >= GUYCAVE) ? 10 : 11 : DMaps[currdmap].color, false,
           darkroom);
      darkroom = newstate;
   }
