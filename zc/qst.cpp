...
int operator ==(DoorComboSet a, DoorComboSet b)
{
   ...
   * if the "for" statement go from 0 to 9. Changing it to backwards
   * solves the issue. */
   for (int i = 8; i >= 0; i--)
   {
      for (int j = 0; j < 6; j++)
...
int readdmaps(PACKFILE *f, zquestheader *Header, uint16_t, uint16_t, uint16_t start_dmap, uint16_t max_dmaps, bool keepdata)
{
   ...
   for (int i = start_dmap; i < dmapstoread + start_dmap; i++)
   {
      ...
      else if (tempDMap.level == 0 && ((Header->zelda_version < 0x211) || ((Header->zelda_version == 0x211)
                                       && (Header->build < 18))))
         tempDMap.flags = dmfCAVES | dmf3STAIR | dmfWHIRLWIND | dmfGUYCAVES;
      else
         tempDMap.flags = 0;
...
int read_one_subscreen(PACKFILE *f, zquestheader *, bool keepdata, int i, uint16_t s_version, uint16_t)
{
   ...
   /* Init the temp_sub buffer to NULL */
   temp_sub.dp1 = NULL;
   ...
   for (j = 0; (j < MAXSUBSCREENITEMS && j < numsub); j++)
   {
      ...
      switch (temp_sub.type)
      {
         ...
         case ssoCURRENTITEMCLASSTEXT:
            ...
            if (!p_igetw(&temp_size, f, true))
               RETURN(qe_invalid);

            if (temp_sub.dp1 != NULL)
            {
               delete[](char *)temp_sub.dp1;
               temp_sub.dp1 = NULL;
            }
      ...
      if (keepdata)
      {
         switch (temp_sub.type)
         {
            ...
            case ssoCURRENTITEMCLASSTEXT:
               ...
               custom_subscreen[i].objects[j].dp1 = temp_sub.dp1;
               temp_sub.dp1 = NULL;
               break;
...
int readguys(PACKFILE *f, zquestheader *Header, bool keepdata)
{
   ...
   if (guyversion > 3)
   {
      ...
      for (int i = 0; i < eMAXGUYS; i++)
      {
         ...
         if (guyversion < 9 && (i == eDKNUT1 || i == eDKNUT2 || i == eDKNUT3 || i == eDKNUT4
                                || i == eDKNUT5)) // Whoops, forgot about Darknuts...
         {
            if (get_bit(quest_rules, qr_NEWENEMYTILES))
            {
         ...
         if (guyversion < 8)
         {
            if (i == eGLEEOK1 || i == eGLEEOK2 || i == eGLEEOK3 || i == eGLEEOK4 || i == eGLEEOK1F || i == eGLEEOK2F
                  || i == eGLEEOK3F || i == eGLEEOK4F)
            {
         ...
         {
            bool boss = (tempguy.family == eeAQUA || tempguy.family == eeDONGO || tempguy.family == eeMANHAN
                         || tempguy.family == eeGHOMA || tempguy.family == eeDIG
            ...
            else if (tempguy.family == eeARMOS)
            {
               ...
               tempguy.misc1 = tempguy.misc2 = tempguy.misc3 = tempguy.misc4 = tempguy.misc5 = tempguy.misc6 = tempguy.misc7 =
                                                  tempguy.misc8 = 0;
         ...
         if (guyversion < 20) // April 2010
         {
            ...
            else if (tempguy.weapon == ewSBomb && tempguy.family != eeROPE && (tempguy.family != eeWALK
