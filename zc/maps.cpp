...
void delete_fireball_shooter(mapscr *s, int i)
{
   ...
   for (int j = 0; j < guys.Count(); j++)
   {
      ...
      if ((int(guys.spr(j)->x) == cx) && (int(guys.spr(j)->y) == cy)
            && (guysbuf[(guys.spr(j)->id) & 0xFFF].flags2 & eneflag_fire))
         guys.del(j);
   }
}
...
void hidden_entrance(int tmp, bool, bool high16only, int single) //Perhaps better known as 'Trigger Secrets'
{
   ...
   for (int i = 0; i < 176; i++) // Do the 16-31 secrets
   {
      ...
      if ((!(s->flags2 & fCLEARSECRET) /*Enemies->Secret*/ && single < 0) || high16only || s->flags4 & fENEMYSCRTPERM)
      {
         ...
         {
            if (t[j].data.empty()
                  || t[j].cset.empty()) continue; //If layer is not valid (surely checking for 'valid' would be better?)
               ...
               if (!(layer->ffflags[i]&ffCHANGER) && (!(layer->ffflags[i]&ffLENSVIS) || lensclk))
               {
                  ...
                  if (!!(layer->ffflags[i]&ffOVERLAY) == (type == -4)) //what exactly is this supposed to mean?
                  {
                     ...
                     if (layer->ffflags[i]&ffTRANS)
                        overcomboblocktranslucent(bmp, tx - x, ty - y, layer->ffdata[i], layer->ffcset[i], 1 + (layer->ffwidth[i] >> 6),
                                                  1 + (layer->ffheight[i] >> 6), 128);
                     else
                        overcomboblock(bmp, tx - x, ty - y, layer->ffdata[i], layer->ffcset[i], 1 + (layer->ffwidth[i] >> 6),
                                       1 + (layer->ffheight[i] >> 6));
                  }
               }
            }
         }
            ...
            if (layer->layermap[type] > 0)
            {
               if (scrolling)
               {
                  ...
                  else
                  {
                     ...
                     {
                        for (int i = 0; i < 176; i++)
                           overcombotranslucent(bmp, ((i & 15) << 4) - x, (i & 0xF0) + PLAYFIELD_OFFSET - y, tmpscr2[type].data[i],
                                                tmpscr2[type].cset[i], layer->layeropacity[type]);
                     }
                     else
                     {
                        for (int i = 0; i < 176; i++)
                           overcombotranslucent(bmp, ((i & 15) << 4) - x, (i & 0xF0) + PLAYFIELD_OFFSET - y, tmpscr3[type].data[i],
                                                tmpscr3[type].cset[i], layer->layeropacity[type]);
                     }
                  }
               }
               else
...
void do_layer(BITMAP *bmp, int type, mapscr *layer, int x, int y, int tempscreen, bool scrolling, bool drawprimitives)
{
   ...
   if (type == (int)(layer->lens_layer & 7) && ((layer->lens_layer & llLENSSHOWS && !lensclk)
         || (layer->lens_layer & llLENSHIDES && lensclk)))
      showlayer = false;
...
void draw_screen(mapscr *this_screen, bool showlink)
{
   ...
   set_clip_rect(scrollbuf, draw_screen_clip_rect_x1, draw_screen_clip_rect_y1, draw_screen_clip_rect_x2,
                 draw_screen_clip_rect_y2);
   ...
   if (!(get_bit(quest_rules, qr_LAYER12UNDERCAVE)))
   {
            ...
            ((Link.getAction() == climbcovertop) || (Link.getAction() == climbcoverbottom)))
      {
         ...
         if (int(Link.getX()) & 15)
         {
            overcombo(scrollbuf, ccx + 16, ccy + cmby2 + PLAYFIELD_OFFSET, MAPCOMBO(ccx + 16, ccy + cmby2), MAPCSET(ccx + 16,
                      ccy + cmby2));
...
void loadscr(int tmp, int destdmap, int scr, int ldir, bool overlay = false)
{
   ...
   for (int j = -1; j < 6; ++j) // j == -1 denotes the current screen
   {
      if (j < 0 || ((tmpscr[tmp].layermap[j] > 0)
                    && (ZCMaps[tmpscr[tmp].layermap[j] - 1].tileWidth == ZCMaps[currmap].tileWidth)
                    && (ZCMaps[tmpscr[tmp].layermap[j] - 1].tileHeight == ZCMaps[currmap].tileHeight)))
      {
         ...
         for (int i = 0; i < (ZCMaps[currmap].tileWidth) * (ZCMaps[currmap].tileHeight); ++i)
         {
            ...
            if (combobuf[c].nextcombo != 0 && (tmpscr[tmp].flags3 & fCYCLEONINIT) && (j < 0
                  || get_bit(quest_rules, qr_CMBCYCLELAYERS)))
            {
               int r = 0;
   ...
   if (tmp == 0)
   {
      for (int i = 0; i < 6; i++)
      {
         if (tmpscr[tmp].layermap[i] > 0)
         {
            ...
            if ((ZCMaps[tmpscr[tmp].layermap[i] - 1].tileWidth == ZCMaps[currmap].tileWidth)
                  && (ZCMaps[tmpscr[tmp].layermap[i] - 1].tileHeight == ZCMaps[currmap].tileHeight))
            {
               tmpscr2[i] = TheMaps[(tmpscr[tmp].layermap[i] - 1) * MAPSCRS + tmpscr[tmp].layerscreen[i]];
