...
enemy::enemy(fix X, fix Y, int Id, int Clk) : sprite()
{
   ...
   if ((wpn == ewBomb || wpn == ewSBomb) && family != eeOTHER && family != eeFIRE && (family != eeWALK
         || dmisc2 != e2tBOMBCHU))
      wpn = 0;
}
...
void enemy::leave_item()
{
   ...
   {
      if (extend >= 3) items.add(new item(x + (txsz - 1) * 8, y + (tysz - 1) * 8, (fix)0, drop_item, ipBIGRANGE + ipTIMER,
                                             0));
...
void enemy::FireWeapon()
{
   ...
   switch (dmisc1)
   {
      ...
      case e1t3SHOTS: //Aquamentus
         Ewpns.add(new weapon(x, y, z, wpn, 2 + (((dir ^ left) + 1) << 3) + (dmisc1 == e1t3SHOTSFAST ? 4 : 0), wdp, dir, -1,
                              getUID(), false));
         Ewpns.add(new weapon(x, y, z, wpn, 2 + (((dir ^ right) + 1) << 3) + (dmisc1 == e1t3SHOTSFAST ? 4 : 0), wdp, dir, -1,
                              getUID(), false));

      //fallthrough
      default:
         Ewpns.add(new weapon(x, y, z, wpn, 2 + (dmisc1 == e1t3SHOTSFAST || dmisc1 == e1tFAST ? 4 : 0), wdp, wpn == ewFireball2
                              || wpn == ewFireball ? 0 : dir, -1, getUID(), false));
         sfx(wpnsfx(wpn), pan(int(x)));
         break;
      ...
      case e1tSLANT:
      {
         ...
         if (((Link.x - x) < -8 && dir == up) || ((Link.x - x) > 8 && dir == down) || ((Link.y - y) < -8 && dir == left)
               || ((Link.y - y) > 8 && dir == right))
            slant = left;
         else if (((Link.x - x) > 8 && dir == up) || ((Link.x - x) < -8 && dir == down) || ((Link.y - y) > 8 && dir == left)
                  || ((Link.y - y) < -8 && dir == right))
            slant = right;

         Ewpns.add(new weapon(x, y, z, wpn, 2 + (((dir ^ slant) + 1) << 3), wdp, wpn == ewFireball2
                              || wpn == ewFireball ? 0 : dir, -1, getUID(), false));
         sfx(wpnsfx(wpn), pan(int(x)));
         break;
      }
      ...
      case e1tSUMMON: // Bat Wizzrobe
      {
         ...
         if (bc <= 40) // Not too many enemies
         {
            ...
            sfx(get_bit(quest_rules, qr_MORESOUNDS) ? SFX_ZN1SUMMON : SFX_FIRE, pan(int(x)));
         }
...
bool enemy::hitshield(int wpnx, int wpny, int xdir)
{
   ...
   if (wpny > y) ret = ((flags & inv_front && xdir == down) || (flags & inv_back && xdir == up) || (flags & inv_left
                           && xdir == left) || (flags & inv_right && xdir == right));
   else if (wpny < y) ret = ((flags & inv_front && xdir == up) || (flags & inv_back && xdir == down) || (flags & inv_left
                                && xdir == right) || (flags & inv_right && xdir == left));

   if (wpnx < x) ret = ret || ((flags & inv_front && xdir == left) || (flags & inv_back && xdir == right)
                                  || (flags & inv_left && xdir == up) || (flags & inv_right && xdir == down));
   else if (wpnx > x) ret = ret || ((flags & inv_front && xdir == right) || (flags & inv_back && xdir == left)
                                       || (flags & inv_left && xdir == down) || (flags & inv_right && xdir == up));
...
int enemy::takehit(weapon *w)
{
   ...
   if ((wpnId == wHookshot && hitshield(wpnx, wpny, xdir))
         || ((flags & inv_front && wpnDir == (xdir ^ down)) || (flags & inv_back && wpnDir == (xdir ^ up)) || (flags & inv_left
               && wpnDir == (xdir ^ left)) || (flags & inv_right && wpnDir == (xdir ^ right)))
      )
   ...
   switch (wpnId)
   {
      ...
      case wHookshot:
      {
         ...
         if (!power) hp -= (enemyHitWeapon > -1 ? itemsbuf[enemyHitWeapon].fam_type : current_item(
                                  itype_hookshot)) * DAMAGE_MULTIPLIER;
         else
            hp -= power;
      ...
      case wHSHandle:
      {
         ...
         bool ignorehookshot = ((defense[edefHOOKSHOT] == edIGNORE) || ((defense[edefHOOKSHOT] == edIGNOREL1
                                || defense[edefHOOKSHOT] == edSTUNORIGNORE)
         ...
         if (!(family == eePEAHAT || family == eeAQUA || family == eeMANHAN || (family == eeWIZZ && !ignorehookshot)
               || (family == eeWALK && dmisc9 == e9tPOLSVOICE) || (family == eeWALK
                     && flags & (inv_back | inv_front | inv_left | inv_right))))
            return 0;
...
void enemy::newdir_8(int newrate, int newhoming, int special, int dx1, int dy1, int dx2, int dy2)
{
   ...
   if (i == 32)
   {
      ...
      ndir = (tmpscr->flags7 & fSIDEVIEW) ? (rand() & 1 ? left : right) :
             -1; // Sideview enemies get trapped if their dir becomes -1
   }
   ...
   switch (anim)
   {
      ...
      case aNEWWIZZ:
      {
         ...
         if (dmisc1)                                           //walking wizzrobe
         {
            ...
            if (!(dummy_bool[1]
                  || dummy_bool[2]))                            //should never be charging or firing for these wizzrobes
            {
               if (dummy_int[1] > 0)
                  tile += 40;
            }
...
waves2:
   ...
   if (extend > 2)
   {
      if (o_tile / TILES_PER_ROW == (o_tile + ((txsz * change) / tilerows)) / TILES_PER_ROW)
         ...
         tile = o_tile + (txsz * change) + ((tysz - 1) * TILES_PER_ROW) * ((o_tile + txsz * change) / TILES_PER_ROW) -
                (o_tile / TILES_PER_ROW);
   }
   else
      tile = o_tile + change;
}
...
int wpnsfx(int wpn)
{
   switch (wpn)
   {
      ...
      case ewRock:
         if (get_bit(quest_rules, qr_MORESOUNDS)) return SFX_ZN1ROCK;

      case ewFireball2:
      case ewFireball:
         if (get_bit(quest_rules, qr_MORESOUNDS)) return SFX_ZN1FIREBALL;
   }
...
bool eLeever::canplace(int d2)
{
   ...
   if (d2 >= left)
      if (m_walkflag(Link.getX(), Link.getY(), spw_halfstep)
            || m_walkflag(Link.getX(), Link.getY() - 8, spw_halfstep))                         /*none*/
         return false;
...
bool eStalfos::animate(int index)
{
   ...
   else if ((hp <= 0 && dmisc2 == e2tSPLIT) || (dmisc2 == e2tSPLITHIT && hp > 0 && hp < guysbuf[id & 0xFFF].hp
            && !slide()))  //Split into enemies
   {
      stop_bgsfx(index);
      int kids = guys.Count();
      int id2 = dmisc3;
   ...
   else if ((clk2 == 16 || dmisc1 == e1tCONSTANT) &&  dmisc1 != e1tEACHTILE && wpn && wpn != ewBrang && sclk == 0
            && !stunclk && !watch)
      switch (dmisc1)
      {
...
int GuyHit(int tx, int ty, int tz, int txsz, int tysz, int tzsz)
{
   for (int i = 0; i < guys.Count(); i++)
   {
      ...
      {
         if (((enemy *)guys.spr(i))->stunclk == 0 && (!get_bit(quest_rules, qr_SAFEENEMYFADE)
               || ((enemy *)guys.spr(i))->fading != fade_flicker)
      ...
      if (!getmapflag(mITEM) && (tmpscr->hasitem != 0))
      {
         if (tmpscr->flags & fITEM)
            ...
            items.add(new item((fix)tmpscr->itemx,
                               (tmpscr->flags7 & fITEMFALLS
                                && tmpscr->flags7 & fSIDEVIEW) ? (fix) - 170 : (fix)tmpscr->itemy + (get_bit(quest_rules, qr_NOITEMOFFSET) ? 0 : 1),
...
{
   addenemy((pos & 15) << 4, pos & 0xF0, (s->cset[pos] << 12) + eSPINTILE1,
            animated_combo_table[s->data[pos]][1] + zc_max(1, combobuf[s->data[pos]].frames));
}
...
void setupscreen()
{
   ...
   switch (tmpscr[t].room)
   {
      ...
      case rSHOP:                                             // shop
      {
         ...
         for (int i = 0; i < count; i++)
         {
            additem((i << step) + base, 89, QMisc.shop[tmpscr[t].catchall].item[i],
                    ipHOLDUP + ipFADE + (tmpscr[t].room == rTAKEONE ? ipONETIME2 : ipCHECK));
            ((item *)items.spr(items.Count() - 1))->PriceIndex = i;
   ...
   switch (MsgStrings[msgstr].s[msgptr] - 1)
   {
      ...
      */
      ...
      case MSGC_GOTOIFGLOBAL:
      {
         ...
         int d = zc_min(7, arg);
         int s = ((get_currdmap()) << 7) + get_currscr() - (DMaps[get_currdmap()].type == dmOVERW ? 0 :
   ...
   if (MsgStrings[msgstr].stringflags & STRINGFLAG_WRAP)
   {
      if (msgspace)
      {
         if (MsgStrings[msgstr].s[msgptr] >= 32 && MsgStrings[msgstr].s[msgptr] <= 126)
         {
            ...
            {
               if (MsgStrings[msgstr].s[msgptr + k] >= 32
                     && MsgStrings[msgstr].s[msgptr + k] <= 126) s3[j++] = MsgStrings[msgstr].s[msgptr + k];
            }
...
breakout:
   ...
   if ((msgpos >= 10000 || msgptr >= MSGSIZE || cursor_y >= msg_h || atend(MsgStrings[msgstr].s + msgptr))
         && !linkedmsgclk)
   {
